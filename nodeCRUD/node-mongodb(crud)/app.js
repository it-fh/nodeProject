...
app.on('request', async (req, res) => {
    ...
    if (method == "GET") {
      if(pathname == '/'){
        ...
        <td>${item.email}</td>
        <td>
            <a href="/remove?_id=${item._id}" class="btn btn-danger btn-xs">删除</a>
            <a href="/modify?_id=${item._id}" class="btn btn-success btn-xs">修改</a>
        </td>
    </tr>
        `;
            })
		...
		<form method='post' action='/add'>
		  ...
		  <button type="submit" class="btn btn-primary">添加用户</button>
           ...
           `;
            ...
            let hobbies = ['吃饭', '足球', '篮球', '橄榄球', '敲代码', '抽烟', '喝酒', '烫头'];
            let modify = `
        <!DOCTYPE html>
<html lang="en">
<head>
...
<body>
 <div class="container">
     <h3>添加用户</h3>
     <form method='post' action='/modify?_id=${user._id}'>
       <div class="form-group">
         <label>用户名</label>
